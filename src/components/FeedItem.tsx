			...
			<VStack p={4} space={2}>
				...
				<HStack space={2}>
					{item?.tags ? item.tags.map((tag, index) =>
						<Pressable key={index} onPress={()=>{navigation.navigate(`SingleTag`, {tag: tag})}}>
							<Text style={styles.tags}>#{tag}</Text>
						</Pressable>): null}
				</HStack>
				<VStack space={2}>
					<Pressable onPress={()=>{navigation.navigate(`SingleFeedScreen`, {feedId: item?._id,})}}>
						<Text style={styles.whiteText}>{item?.string_story}</Text>
					</Pressable>
					<Flex wrap="wrap" direction="row">
						{item?.images ? item.images?.map((item, index)=>(
							<Box style={isDoubleContentLayout ? styles.doubleContent : isTripleContentLayout? styles.tripleContent: styles.singleContent} m={0.5} key={index}>
